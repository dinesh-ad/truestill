...
| **Tests** | 491 Python + 16 browser end-to-end. All four CI lanes green. Assert behaviour, never counts - these numbers are context, not a gate, and **must not be pasted into a doc as a target**. Re-derive with `uv run pytest --collect-only -q`. |
...
### 2.2 CURRENT ARC: the tab-tour findings - **Stage 13.2 is BUILT; 13.3 next**
...
1. **Stage 2d - the review stage and layout wiring - IN PROGRESS.**
   ...
   fixture and an explicit STOP point each - read there before starting any of it. **13.0-13.2**
   are built - see the Stage rows above. **Next: 13.3**, the review UI (propose, adjust edges,
   name, skip - catalog only, no relocation). Then 13.4 (migration wiring, which widens `(mm)`'s
   collision-scoping boundary).
