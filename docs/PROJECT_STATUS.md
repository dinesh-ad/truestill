...
| **Tests** | 502 Python + 16 browser end-to-end. All four CI lanes green. Assert behaviour, never counts - these numbers are context, not a gate, and **must not be pasted into a doc as a target**. Re-derive with `uv run pytest --collect-only -q`. |
...
### 2.2 CURRENT ARC: the tab-tour findings - **Stage 13.3b is BUILT; 13.4 next**
...
| **Stage 13.3a** | Reveal in file manager, on the apply-to-disk result - frontend wiring, **no new endpoint**. The aggregate "Moved N photos into trip folders." line is now one row per named+applied event/trip this run, each with its real folder and a working reveal link, reusing the `[data-open]`/`/api/reveal` pattern already shipped for Backups drive cards unchanged. Two things originally specified were considered and **rejected**, not built: a root-restriction on `/api/reveal` (withdrawn - `LocalGuard`'s token/Host/Origin checks already cover the actual threat in `SECURITY.md`'s model, and every other filesystem endpoint - `fs_dirs`/`fs_validate`/`fs_create` - is equally unrestricted by design; restricting reveal alone would have broken the shipped Backups use, which points outside any library root on purpose) and a disabled-for-not-yet-applied card state (withdrawn - no per-trip card with a path exists anywhere in the shipped UI to attach one to; proposal cards never have a folder, and the post-apply view had no per-trip breakdown until this stage added one). One backend touch: `Catalog.sample_relative_for_event` + an optional `named_events` param on `migration_apply`, unused (and so unaffected) by the plain Settings-screen migration. |
| **Stage 13.3b** | The proposal-card redesign - the inversion, split/merge kept as a pair, EVENT vs TRIP labelling. `trip_review.assemble_trip_review` runs `detect_trips` (2b) first, so a genuine multi-day run renders as **one** card and a standalone day still renders as its own (unchanged) day-event card, instead of the user reassembling by hand what detection already knew. A card's `kind` ("trip" \| "event") is a display label by day count, resolving the "Trips & events" naming collision (13.1). SPLIT stays primary (`trip_review.split_trip` for a trip, `events.split_candidate` unchanged for an event); MERGE stays secondary and now always produces a `TripProposal` - never a raw concatenated `EventCandidate` - because a manual merge must obey the same two locked rules detection does (§3e year boundary, §3f max-span), refusing/declining with a stated reason rather than fabricating or truncating a trip. Nothing is auto-applied: a trip is written only on explicit name + confirm, through 13.1's `commit_trips`. **Supersedes** 13.3's originally-sketched edge-trim/extend interaction model - the actual instruction required mirroring the shipped day-event handlers, not inventing a new gesture; flagged in `trip-grouping-research.md` §13.3b rather than silently overwritten. **A flagged consequence**: because merging must now obey §3e/§3f, merging two day-events always yields a trip, and a trip does not reach `migrate.py`/apply-to-disk until 13.4 - the app says so honestly rather than reporting a false "already moved". Also **completes** Stage 3's "a trip offered as one proposal rather than one per day" item (§3 below) ahead of schedule. |
...
1. **Stage 2d - the review stage and layout wiring - IN PROGRESS.**
   ...
   fixture and an explicit STOP point each - read there before starting any of it. **13.0-13.3b**
   are built - see the Stage rows above. **Next: 13.4**, migration wiring (the confirmed-trip
   apply-to-disk step 13.3b deliberately does not reach; also widens `(mm)`'s collision-scoping
   boundary).
