...
| **Your photo data never leaves your machine.** No telemetry, no usage beacon, no phone-home in the CLI, core, or app, and **nothing about a library** - not filenames, not counts, not hashes, not contents, not paths - is ever transmitted. **ONE EXCEPTION, AND IT IS NAMED RATHER THAN IMPLIED (maintainer's ruling, 2026-08-10): a FOLDER NAME may be sent to a naming service the user explicitly chooses.** Per use, never by default, never automatically, and never carried along by any other operation - an organize, a backup, a migration or a verify transmits nothing, whatever the user chose for naming. A folder name and nothing beside it: not the path it sits in, not the files under it, not how many, not any hash. **The absolute is narrowed here, not weakened, and the difference is that the hole has edges:** everything outside this one sentence is still never transmitted, and a second exception would need its own ruling and its own line rather than reading as covered by this one. *Why it had to be written before anything was designed: a naming layer that sends folder names was designed across two sessions while this row said filenames are never transmitted - the design was forbidden by the contract that overrides everything, and nobody had read the sentence (`ENGINEERING_STANDARD.md` §4, thirty-third member).* An **account is required at activation** (`DECISIONS.md` D5, which supersedes D1): one-time, email-verified against a self-hosted licensing server, after which the app holds a signed local token and runs fully offline. Usage is otherwise measured externally and in aggregate. Any future crash reporting must be opt-in, off by default, self-hosted, transparent, and post-launch only. | No network path in the product transmits user activity or library content; the capability seam (`§2`) gates on the local token. **Unbuilt** - post-launch, and the licensing server needs its own design pass first. |
...
- **Migration ledger:** v2 `size`, v3 `original_name`, v4 event tables (`events` +
  ...
  *"re-copy the source to restore a bad file"*, **which discards the user's confirmed date and
  ...
  v23 `authored_decisions` - **the fact a lease needs, and the reason it is a table**, `(aix)`.
  A rename changes a name the drive's decisions document already holds, and `would_lose` counts a
  **changed** value as a loss (`(ahz)` step 3, written after a drive holding a real name was
  overwritten by a catalog holding a placeholder). That guard cannot tell a deliberate rename from
  a rebuilt catalog that never knew the name - **only the writer knows which** - so the rename
  records what it authored and the publish path reads it.
  🔑 **A LEASE, NOT A FORCE FLAG, and `expected` is the difference.** Each row is
  `(section, key) -> the value the renamer expects the drive to still hold`, so a write is a
  compare-and-swap: a name changed on **another machine** fails the comparison and survives. This
  is git's `--force-with-lease` (*"an atomic compare-and-swap on the branch you are pushing to"*)
  rather than `--force` (*"has really no checking"*), and the per-key scoping is the **field mask**
  pattern Google's AIP-134/161 require over an unscoped `force=true` boolean.
  ⚠ **The row is written in the SAME TRANSACTION as the name flip**, so no crash can leave a
  flipped name with no lease - the state that would refuse the user's own rename forever. And
...
| **Real-library corpus fence (2026-07-30; restated as an instruction 2026-08-10).** Test / profile / soak against **only** the maintainer's stated pair: source `TruestillLibrary/Input`, destination `TruestillLibrary/Output`. `Input/Testing-new` stays out - single copy, uncatalogued. **THE ENCRYPTED CLOUD MOUNTS ARE FENCED OUTRIGHT, AND THEY ARE NAMED: `/home/dinesh/pCloudDrive/` and `/home/dinesh/Icedrive/`. Never read, never walked, never stat'd - at any depth, under any flag.** No `find` is ever rooted at `/home/dinesh`, because a bare `find /home/dinesh -maxdepth 4` walks both. Root every search at the repo or at `/home/dinesh/TruestillLibrary`. ⚠ **TIGHTENED 2026-08-14, and the previous rule is quoted so the change is visible rather than silent.** It read: *"The maintainer's encrypted cloud mount is READ-ONLY. Resolving, stat-ing and reading a path to understand structure is allowed; writing to it, organizing into it, or walking it broadly is not."* Two things were wrong with it. **It named no path**, so the fence could not be checked against a command before that command ran - and a rule you cannot check before acting is a rule you discover you have broken. **And the permission it granted is not cheap**: both are FUSE mounts backed by a network service, so a `stat` is a billed round-trip, not a local syscall. The cost is invisible in the command and real on the wire, which is exactly the shape of thing a written rule has to carry because nothing in the terminal will show it to you. Reading is for understanding a path, never for scanning a library - **if something would read more than a handful of entries, ask first.** **The locked folder inside that mount is OFF LIMITS, unconditionally**: do not resolve into it, do not stat inside it, do not descend. There is **no task-scoped exception**; if one is ever needed the maintainer grants it himself, in his own words. **The reason, which is what makes the rule predictable rather than arbitrary:** the cost is his disk. A scan there pulls gigabytes through the local cache. It was never about protecting the cloud from us. **This row said something different until 2026-08-10, and how it failed is the point:** it asserted the client was uninstalled and the material *"cannot be reached by any means"* - true when written, false the moment he reinstalled, and nothing could notice. A binding clause states **intent**, which stays true, not a **machine state**, which expires in silence (`ENGINEERING_STANDARD.md` §4, thirty-second member).  ⚠ **AND THE OTHER HALF OF THE FENCE, RECORDED 2026-08-15: `Input` and `Output` ARE FREELY WRITABLE SCRATCH, ON THIS MACHINE ONLY.** The maintainer has ruled that copies, messy folders and leftover test output in either are fine - he does not care what is in them. Two consequences that are easy to miss and expensive to learn: **(1)** they do not exist on another machine or for another developer, so **nothing may be a fixture, a test dependency, or a design premise on the strength of what they contain**; and **(2)** any measurement taken from them is a **snapshot, not a fact** - a single test run can invalidate it, and one did. `(abg)` recorded `Output` as empty and unmarked on 2026-08-10 and it now holds 338 media files from grid testing, which changed what an open backlog entry appeared to be about. **Read them freely; cite them with a date; design against the catalog, which reproduces anywhere.** If a task appears to need more than this allows, STOP and ask. | Convention - human process, **now with two enforcement layers behind it (installed and TESTED 2026-08-23)**: `permissions.deny` rules (`Read`/`Edit` on both paths, double-slash absolute form) and `sandbox.denyRead` in the maintainer's `~/.claude/settings.json`. A `Read` probe was refused mid-session (*"denied by your permission settings"*) and a bash probe was refused before execution. ⚠ **Honest limits, so nobody over-trusts it**: both layers live in user settings **outside this repo** - a fresh clone on another machine has the convention alone; permissions cover Claude's file tools, the sandbox covers bash and its children, and MCP/hooks are covered by neither - so **the convention stays the rule and the layers are backstops**. 🔑 The strongest control is free: both mounts were verified **unmounted** during agent sessions (2026-08-23, `/proc/mounts` - the fenced paths are empty local directories until the FUSE daemons run), and not mounting them during sessions beats any rule. **This row is the source**; `PROJECT_STATUS.md` §4 carries the short form and points here. It used to cite §4 as *its* source, which is how one stale sentence became two documents saying it. |
| **Format-variety corpora (2026-08-21). `/home/dinesh/TruestillLibrary/` is FREE SCRATCH in its entirety** - nested folders, N copies, deliberately messy trees, anything, no permission needed - and two further corpora are **real git repos** under `~/ad/application/`, outside the library and outside every fence: **`exif-samples`** (ianare, 115 files / 54 MB) and **`metadata-extractor-images`** (drewnoakes, 10,703 files / 2.8 GB across 52 format directories). Copy from them into `TruestillLibrary` freely. ⚠ **THEY ARE A DIFFERENT AXIS FROM A REAL LIBRARY, AND THAT IS THE ENTIRE POINT.** Soak one was 4,111 files from **one person's devices**, so it covers the formats those devices emit and nothing else - a maker note that parses wrong, an orientation tag in an unexpected place, a container exiftool reads differently **cannot appear there no matter how large it gets**. The precedent is `(adp)`: 33% of a real corpus drawn sideways, found only by rendering real photographs. This is the same move aimed at **parsing** rather than rendering. Measured 2026-08-21: the two hold **1,428 files Truestill would organize across 38 distinct media extensions**, against a real library that is overwhelmingly one lineage of `.jpg`; they also hold **1,461 deliberately fuzzed files**, which it declines by extension today. **The snapshot rule still applies to `TruestillLibrary`** - anything observed there is a snapshot, never a fixture and never a design premise. The two repos are different in kind: they are **version-controlled and reproducible on any machine**, so a finding against them can be cited by commit rather than by date. | Convention - human process. The fence row above remains the source for `TruestillLibrary`, and **this row adds corpora while removing no restriction from it** - the mounts that row fences outright stay fenced, and are named there rather than repeated here. |
| **An entry that leaves `BACKLOG.md` is closed by the commit that moves it.** The message declares `Closes (xyz)` on a line of its own and the entry arrives in `SHIPPED.md` in the same commit - a ruling in conversation is not a closure until a commit records it. The full stop is conventional and not required, because requiring it would fail silently in the direction that matters. | **Enforced, in two places, because only one direction is checkable against history:** `test_closed_entries_leave_the_backlog.py` (corpus - declared closed, yet still open work or in neither document) and `scripts/check_entry_closure.py` via the `commit-msg` hook (per-commit - left the backlog undeclared). The hook reads only the staged diff, so its boundary is structural rather than a date; the corpus form would fail all but one of `SHIPPED.md`'s entries (**31 of 32** when that was written, and the ratio is the durable part). ⚠ This clause carried the live count as a frozen number - *"48 entries today"* - and it was wrong within a day of being written, because `(adk)` shipped. Count it instead: `grep -cE '^- \*\*\([a-z]{1,3}\)' docs/SHIPPED.md`. Full reasoning in `BACKLOG.md`'s *Item letters*. |
| **Commit identity / no-AI-trailer** - no `Co-Authored-By` trailer, no Anthropic/Claude email or signature in history. **Rule source:** [`GOLDEN_REPO_RULES.md`](GOLDEN_REPO_RULES.md) §1 (portable across product repos). | **Enforced here:** `scripts/check_commit_msg.py` via the `commit-msg` pre-commit hook (`.pre-commit-config.yaml`, id `no-ai-coauthor`). Activate: `uv run pre-commit install --hook-type commit-msg`. Pre-commit floor and install stages: same file §2. |
...
- **`make gate`** = `check`, then `e2e` **only when the diff reaches the browser**. It prints
  which files decided it, either way. ⚠ **NOT PART OF THE ROUTINE LOOP SINCE 2026-08-20** - see
  6.1. The command is unchanged; when to run it is not.
...
findable on the maintainer's machine.
...
band at both ends - and the browser lane runs **1169-1391 s** on CI over eight runs and **1475 s**
locally, against a *"~6:50"* (410 s) that was **3x low**. The rows now carry the **ceiling**, which
is the number a reader actually needs and the only one this file owns: `TEST_SECONDS_MAX` and
`E2E_SECONDS_MAX` are in the `Makefile`, so a ceiling here cannot drift from the thing it names.
This is the rule directly below applied to itself - *a duration and its ceiling, never a test
count* - which stopped one step short: **the duration rots too.**

⚠ **AND A MEASUREMENT IN `PERFORMANCE.md` OUTRANKS A DURATION QUOTED HERE.** *"The contract wins
on any conflict"* is the repo's standing rule, and on 2026-08-15 it pointed the wrong way: this
file said the browser lane cost ~6:50 while `PERFORMANCE.md` said 18.6 min, and **`PERFORMANCE.md`
was the correct one**. The rule holds for *rules* - what is required, forbidden, or binding - and
inverts for *measurements*, because `PERFORMANCE.md` is where a measurement is maintained and
anything here is a copy. **A copy of a number cannot be more current than its source.** So: cite
`PERFORMANCE.md` for what a lane costs; state ceilings and requirements here.
...
| `pillow>=12.3.0` (`truestill-core`) | Image decoding backing imagehash and cheap dimension reads. **Large-image policy:** truestill processes the user's own local library (trusted), not untrusted uploads, so Pillow's ~89 MP decompression-bomb guard is a false positive on legitimate large photos (panoramas/scans). `hashing.MAX_PERCEPTUAL_PIXELS` raises `Image.MAX_IMAGE_PIXELS` to **300 MP** deliberately. ⚠ **THE SKIP IS AT 600 MP, AND THIS ROW SAID 300 UNTIL 2026-08-26 - it was never true.** Pillow's `_decompression_bomb_check` is **two tiers**: it *warns* above `MAX_IMAGE_PIXELS` and only *raises* above **twice** it, so between 300 and 600 MP an image is warned about and **hashed anyway**. Above 600 MP it is **skipped for perceptual hashing** (SHA-256 exact dedup still applies) and the bomb *warning* is suppressed locally so **no raw Pillow warning reaches the terminal**. Whether 300 or 600 is the intended ceiling is **unruled** and is `(aib)`; the 300-600 MP band is empty in practice - all 5 files in the format corpus claiming over 300 MP claim over 600. (Immich/PhotoPrism avoid this entirely via libvips streaming.) |
...
| `pynacl>=1.6.2` (`truestill-core`) | **Ed25519 signature verification for licence tokens** (`licence.py`), which the stdlib does not expose - `hashlib` and `hmac` are symmetric, so the only stdlib route is hand-rolling curve arithmetic, and hand-rolled signature verification is the class of code where a subtle bug is a forged licence rather than a crash. `DECISIONS.md` **D6 §2** chose the algorithm; this row is the choice of **binding**, and it is a **D7** question because D7 forbids a copyleft dependency in a published package without a superseding decision. **Apache-2.0** (PyPI metadata, checked 2026-09-11) - the same licence as this project, so D7 is satisfied outright. The alternative, `cryptography`, is `Apache-2.0 OR BSD-3-Clause` and **also clears D7**; it was refused on **surface**, measured rather than felt: **OSV lists 42 advisories against `cryptography` and 2 against `pynacl`** (2026-09-11), and both of pynacl's are the same libsodium issue fixed in **1.6.2**, which is this floor. The asymmetry is structural rather than luck - `cryptography`'s wheels **statically link OpenSSL**, so every OpenSSL advisory becomes a `cryptography` advisory and a `pip-audit` red here; this lane went red on transitive CVEs on 2026-09-09 and taking on a bundled TLS stack to verify a 64-byte signature buys that class of red permanently. Also **1.4 MB against 4.7 MB** (manylinux x86_64), which an installer for non-technical users carries. Both ship abi3 wheels covering 3.14 on every platform D9 publishes. ⚠ **The counter-argument, recorded rather than omitted: pynacl went 3.5 years between 1.5.0 (2022-01) and 1.6.0 (2025-09).** It is maintained by PyCA - the same organisation behind `cryptography` - and has shipped three releases in the four months since, so it is not abandoned; if it stalls again the exit is cheap, because `licence.py` calls exactly two of its functions. Brings `cffi` and `pycparser` transitively. |
...
| **The text-size preference ADJUSTS the reader's own default and never replaces it.** `tokens.css` declared no root font-size and said why - "the root is the user's" - and ended "which is why there is no font-size setting in the product". That held against an *absolute* setting only. The case it did not cover is wanting text **smaller** than the browser default, where zoom is a poor answer because it scales the layout with the type. So the steps are **percentages** (75% / 125%) and **medium declares no rule at all**: the absence of a declaration is the mechanism, and `100%` would look equivalent while making medium a value the app asserts rather than one it declines to. A reader whose own default is 24px gets 30px on Large, never 20. **Bounded on purpose** - a larger multiplier compounds on a default that is already raised, which is the state this setting is most likely to be used in; 30px is therefore the largest root the app can be asked for, past the 24px the type-scale tests already prove. **This row said 87.5% / 112.5% and 27px until 2026-08-13, and how it drifted is the point rather than the correction.** The band was widened to 75% / 125% in `f953b25` because +/-12.5% satisfied "small < medium < large" perfectly and read as nothing happening - the maintainer changed the setting and saw no effect. That commit touched this file and not this row, so the binding contract stated a number the product had stopped using, and `COMPOUND_WORST_CASE_PX` in the browser test inherited it: the guard named itself the worst case while testing three pixels short of one. The property held at both - measured 0px custody overflow and 0px page overflow at 27px **and** at 30px - so only the numbers were wrong, which is exactly why nothing went red. **Nothing but type moves**: `--sidebar-width` (272/64px), `--icon-size` (16px), `--space-*` and the 720px media query stay px, so the setting moves exactly the lever a raised browser default already moves and nothing else. **Where the claim is settled matters**: a browser test cannot raise the root except by an inline style, which beats the stylesheet, so such a test reads back its own input and passes for an absolute implementation too - the percentage is asserted in the declaration instead. Values are normalised **server-side on both directions**; an unrecognised stored value resolves to medium rather than reaching the root as an invalid `font-size` the browser drops silently. | `service/organize.TEXT_SIZE_KEY` / `TEXT_SIZES` / `_normalize_text_size`; `/api/text-size/settings`; `static/tokens.css` `:root[data-text-size]`; `static/app.js` `applyTextSize` / `loadTextSize`. Pinned by `test_text_size_setting.py` (service) and `tests/e2e/test_text_size_setting.py`. |
...
| **A drawing constant is never the number in a claim.** The rail's custody pips are three glyphs, so the count they draw is clamped to three - and that clamp was also the number the sentence was written against, so a library with every file on four drives read *"every file in 3 places"* while the panel, reading `held_floor` straight, said four: two surfaces disagreeing in one render, and the understating one was the sentence shown on every screen. **Understating is still misstating** - the same defect as `(acq)`'s per-drive count, arriving from the presentation side instead of the query. The pips keep the clamp, the sentence states the floor, and it goes through `plural` rather than a glued `places` so a bound that today makes "1 place" unreachable is not something the string has to trust. | `static/app.js` `loadCustody` - `filled` (pips) vs `heldFloor` (sentence). Pinned by `test_the_sentence_states_the_floor_the_pips_can_only_draw_three_of` and `test_the_counted_wording_states_the_floor_too`, at a floor of four because every earlier fixture sits at one or two where the clamp cannot bite. The clamp and the three-slot row are **overlapping defences**: removing either alone still draws three, so the pip half of that test is honestly a guard on the pair (§4's eighth member). |
...
| **An OBSERVED path is a hint and is cleared when it goes; a DECLARED path is a stated intent and is never auto-cleared.** The two are different kinds of fact and must not share a mechanism. `take_live_path_hint` deletes a hint whose path is not currently reachable, which is right for "where a run wrote" and **wrong for "where the user said their library lives"** - storing a declaration that way makes an unplugged drive erase the answer, and first run asks again. Reachability is reported *beside* a declaration, never folded into it, so a screen can say *not reachable right now* instead of *never chosen*. Added 2026-08-12 with `(abx)`; it completes §3.1's *identity is never a path* rather than adding a rule, by naming the one path-shaped thing that **is** a user's answer. | `LIBRARY_ROOT_KEY` (`library.root`) read with a plain `get_setting`, never through `take_live_path_hint`. **Load-bearing test:** `test_the_declared_root_survives_its_path_becoming_unreachable` asserts both halves on one vanished path - the declaration stands, the hint beside it is cleared. Do not weaken it. |
| **A place the walk did not enter is NAMED, never counted, and the reason drives the remedy from one place.** A folder of 21 photos, 18 of them inside `.MyAlbum`, reported *"analysed 3 · organized 3"* and **success**. Two rules, learned separately: **files are counted, folders are named without a count** (`c027dd3`), because the walk never descends and the number inside is precisely what is unknown - so *"1 hidden folder"* is a refusal to invent, not an understatement, and `folders: tuple[str, ...]` carries **no integer beside it** so the type refuses the fabrication a docstring could only discourage. And **one entry per REASON**: the two facts are the same shape, what differs is why and what to do, which is a value rather than a field - two flat lists would give a third reason a third renderer branch and a third chance for one surface to miss it. ⚠ **The remedy is worded in `models` and nowhere else.** It existed three times before this - twice verbatim in the CLI and once in `app.js` **worded differently** (*"then preview again"* against *"then run again"*), so the shared string is **surface-neutral** (*"try again"*) and the browser maps no reason to a sentence. ⚠ **A FOLDER IS OPENED, A FILE IS READ**, and sharing one string is what forced the choice: the CLI printed *"folders that could not be read"* above its own *"files that could not be read: N"*, one phrase for the counted fact and the uncountable one. The browser's verb won. **The browser lane is what caught it** - 2,648 pytest cases were green. | `models.FolderSkip` / `folder_skip_label` / `folder_skip_remedy`; `organizer.SkippedFolderGroup` / `skipped_folder_groups` / `FOLDER_PREVIEW`; rendered by `cli._print_folder_groups` and `frontend/src/inventory.tsx` `UnreadableBanner` (was `app.js` `renderUnreadable` until 2026-09-05). Pinned by `test_the_skipped_report_names_hidden_files_and_folders.py` and `test_the_browser_holds_no_folder_wording.py`, which asserts the browser holds neither string **by checking `models`' real ones** rather than a remembered phrase. |
| **ABSENT AND REFUSED ARE DIFFERENT ANSWERS, AND THE DISTINCTION IS MADE IN ONE PLACE FROM ONE `stat`.** A path that does not exist can be created; one that refuses to be described cannot, and the create fails exactly as the probe did - so reporting refusal as absence sends a user round a loop. ⚠ **Never `Path.is_dir()`/`exists()`/`is_file()`**: they stopped raising on `EACCES` in Python 3.14 (`is_dir()` is now `return os.path.isdir(self)`), and `os.path` has always swallowed `OSError` - pathlib was the outlier. `path_reach.probe` takes **one** `stat`, hands it back so a caller needing `st_dev` does not take a second, and classifies with **CPython 3.13's own `_IGNORED_ERRNOS`/`_IGNORED_WINERRORS`**, copied deliberately so the answer is unchanged on the version we ship. ⚠ **`reclaim` is a named exception, not a gap**: it deletes files, so *not there* and *I could not look* must both mean no, and it must not acquire a distinction it would have to discard. | `truestill_core.path_reach` (`probe` / `reach` / `Reach`); read by `path_probe.probe_dir` and `nearest_device`, `LocalDestination.exists`, `date_rescue`, `drive_adoption`. Pinned by `test_refused_is_never_absent.py`, whose fixture makes the predicates **swallow** - simulating the new stdlib rather than the old one - and by `test_reclaim_never_deletes_what_it_cannot_examine.py` and `test_cleanup_never_removes_what_it_cannot_examine.py` for the two delete-adjacent modules. ⚠ **A DELETE PATH MAY NOT GATE ON A BARE PREDICATE AT ALL** - not even one that collapses refusal safely - because the failure is that it **raises**: `reclaim._verify` and `cleanup.plan_cleanup` both died with an uncaught `PermissionError` on 3.13 while the guarded call sat feet away. Swept 2026-08-21: **two of the three** delete-adjacent modules carried it; `organizer._move_source` is the correct shape, verifying by **checksum** rather than by a predicate and catching around both the verify and the `unlink`. |
| **A PATH THE CATALOG RECORDED AS A DRIVE'S HOME IS REFUSED, NEVER OFFERED REGISTRATION - AND THE DISCRIMINATOR IS A RECORDED EXPECTATION, NOT DETECTION.** A cleanly unmounted mountpoint is byte-for-byte an ordinary empty directory: `os.path.ismount` is `False`, `st_dev` equals the parent's, and it is absent from `/proc/mounts` - on Linux and macOS alike, and Windows has no such state. **No probe can tell them apart**, which is why administrators set `chattr +i` on empty mountpoints by hand. Only `path_hint.drive.<uuid>` discriminates, so **every command that resolves a drive root asks `ghost_drive_at` before suggesting `--init`**, and where nothing was recorded the message names **both** readings rather than instructing one. ⚠ **A location hint is a WRITE**: it goes through `set_local_setting` so it cannot dirty the catalog and fire a decisions save to every reachable drive, and it is recorded only where the command already writes drive facts - a preview must not gain a side effect. | `drive.ghost_drive_at` / `ghost_drive_refusal`; `cli._drive_or_explain` / `_init_drive` / `remember_drive_root`; `Catalog.set_local_setting`, which **refuses** a key the decisions document would carry. Pinned by `test_a_ghost_drive_is_never_offered_registration.py`, `test_local_settings_match_the_document_exclusions.py`, and the two existing guards this had to satisfy - `test_catalog_opens_go_through_the_session.py` and `test_every_hint_write_checks_for_a_second_place.py`. |
| **A LIBRARY'S DIAGNOSTIC IS NEVER THE PRODUCT'S OUTPUT, AND WHAT THE USER IS TOLD IS THE CONSEQUENCE.** One `organize` put **866 lines** on stderr - 133 raw `UserWarning`s naming `site-packages/PIL`, and ~598 written by libtiff/libjpeg straight to **file descriptor 2**. ⚠ **But the warnings were the wrong subject**: measured, **478 photos got no near-duplicate check and only 71 warned**, while **14 warned and decoded perfectly well**. Reporting the warnings would have named 71 of 478 and implied the rest were fine; suppressing them alone would have made the product quieter about a real gap. So the line a person reads is derived from the decode **outcome**, never from whether a library spoke - and the removed lines are **counted and stated**, so *quiet* and *nothing happened* stay distinguishable. ⚠ **`warnings.catch_warnings` MUST NOT BE USED FOR THIS**: it assigns process-global module attributes and hashing runs on a thread pool by default, which CPython documents as *undefined behaviour*. One install per process, at a phase boundary, never per file. **Per-file attribution of the C half is impossible** - fd 2 is process-wide and the lines name no file - so it is a phase-level count and the report says so. | `decode_noise.install` / `capture_decoder_output` / `snapshot`; `organizer.uncompared_photos`; `models.UncomparedReason` / `uncompared_label` / `uncompared_remedy`. ⚠ **This row named `models.UNCOMPARED_LABEL` until 2026-08-27**, a module constant that no longer exists: `(ahq)`'s never-silent half turned one heading into a **two-member table** (`UNDECODABLE`, `NO_SIGNAL`) because a file can now miss the comparison for a second reason - it decoded perfectly and carries no distinguishing detail. The rule above is unchanged and the report is still derived from the OUTCOME; only the vocabulary grew a member. Pinned by `test_decode_noise_never_mutates_global_state.py` - which asserts `warnings.filters` is **identical before and after** 8 threads decode, forces the child test onto **spawn** because Linux's `fork` hides the initializer, and proves the fd-2 restore in a **subprocess** because pytest has already replaced fd 2. | 
