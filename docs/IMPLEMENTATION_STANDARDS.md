...
| **A relocation is gated on an explicit typed word, never a default-yes.** `--apply` is permission to *ask*, not permission to move: `migrate-layout` prints the plan and requires the exact word `move`; anything else (including a bare Enter) aborts with nothing moved. Every migration or Trips & events preview is a read: neither the CLI nor the app may write anything at all, not even refresh a connected drive's label or `last_seen`. | `cli._cmd_migrate_layout`, `service.migration_preview`, `service.propose_events`; pinned by `test_a_preview_moves_nothing_and_writes_nothing`, `test_without_the_typed_confirm_nothing_moves` and `test_drive_preview_endpoints_never_refresh_the_catalog`. |
...
| **The Trips & events size floor is a per-catalog setting, default 8.** Lower values expose more small suggestions; higher values hide them and must be an explicit user choice. An unset catalog resolves through `events.DEFAULT_MIN_FILES`, never a second hardcoded default, and the resolved value reaches the same `cluster_camera(min_files=...)` call that gates both trip and standalone-event proposals. | `events.EVENT_MIN_FILES_KEY`; `service.event_settings` / `set_event_settings` / `propose_events`; `trip_review.assemble_trip_review`; pinned by `test_event_min_files_setting_changes_proposals_and_unset_keeps_default`. |
