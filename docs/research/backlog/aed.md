...
*Body of backlog entry `(aed)`, under **Records - evidence, explicitly not work**. The index is [`BACKLOG.md`](../../BACKLOG.md); the letter namespace is shared with [`SHIPPED.md`](../../SHIPPED.md).*
...
- **(aed) THE METADATA BAKER STAGES EVERY BAKED FILE THROUGH THE SYSTEM TEMP DIRECTORY.** Recorded
  ...
  - **What it is.** `organizer._MetadataBaker` (`organizer.py:_MetadataBaker`) stages into the **system** temp
