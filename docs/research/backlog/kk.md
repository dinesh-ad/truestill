...
*Body of backlog entry `(kk)`, under **Conditional, and counted**. The index is [`BACKLOG.md`](../../BACKLOG.md); the letter namespace is shared with [`SHIPPED.md`](../../SHIPPED.md).*

- **(kk) Persist `GPSDateStamp` at ingest - the coordinates half shipped at v17.** Found while designing trip
