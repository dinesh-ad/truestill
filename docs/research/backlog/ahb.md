...
*Body of entry `(ahb)`, under **Conditional, and counted**. The index is [`BACKLOG.md`](../../BACKLOG.md); the letter namespace is shared with [`SHIPPED.md`](../../SHIPPED.md).*
...
(`cli.py:_has_move_semantics`) covers only the `--skip-undated` case - files *not copied at all* - which is a
