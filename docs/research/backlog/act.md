...
*Body of backlog entry `(act)`, under **Internal / tooling**. The index is [`BACKLOG.md`](../../BACKLOG.md); the letter namespace is shared with [`SHIPPED.md`](../../SHIPPED.md).*
...
- **(act) AN UNNAMED ROOT IS LABELLED WITH THE LITERAL STRING `Library`, WHICH COLLIDES WITH
  ...
  - ⚠ **RE-COUNTED 2026-08-22: FIVE SITES, NOT THREE**, and the line numbers below have all moved
    - `service/drives.py:attach_drive`, `:382`, `:383`, `service/destination_scope.py:scope_to_marker`, `cli.py:_print_ingest_report`. The
  ...
  - Three of the four registration sites mint `label=path.name or "Library"`
    (`service/drives.py:attach_drive`, `service/destination_scope.py:scope_to_marker`, `cli.py:_print_ingest_report`).
