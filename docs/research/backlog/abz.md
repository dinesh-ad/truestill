...
*Body of backlog entry `(abz)`, under **Approved - still to build**. The index is [`BACKLOG.md`](../../BACKLOG.md); the letter namespace is shared with [`SHIPPED.md`](../../SHIPPED.md).*

- **(abz) Organize shows one population three ways and connects none of them.** Recorded
  2026-08-08, editorial. On one flow: *"2109 photos and videos here"* (`app.js:2029`, from
  `/api/fs/validate`), then *"2,106 photos - 3 videos found"* (the preview summary, split by
  kind), then *"2,109 duplicates"* (`app.js:2639`, `s.exact_dup`). `2106 + 3 = 2109`: the SAME
