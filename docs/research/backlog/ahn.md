...
- **(ahn) THE PAYLOAD CONTRACT STOPS AT THE PYTHON BOUNDARY, AND REACT IS BEING BUILT AGAINST
  ...
  **So msgspec replaces the type-to-schema half of 4c** with a library call plus that pass, and
  ...
  | **D · the join + emission** - `oneOf` of `$ref`s per route, `GET`+`POST` as two operations, the events route as `text/event-stream`; `openapi.json` committed; a Python-only regenerate-and-diff test in `make check` | the spec is the artefact and drift is red, in the lane with no Node | ✅ **`7b476d9`, 2026-09-03**: `packages/truestill-app/openapi.json` committed; `scripts/emit_openapi.py --write` regenerates, `--check` and `test_the_committed_spec_is_current.py` refuse drift; deterministic. The derivations moved to `scripts/payload_contract.py`; each arm carries its status and method. **No union can carry an OpenAPI discriminator** - none shares a string tag; 13 two-member unions share a boolean one, 10 share nothing, every job envelope among them because `JobStarted` is `{"job_id"}`. Every arm of a status is one `oneOf` (111 arms asserted into the document, 38 union operations on 200; 409, 400 and 503 arms kept apart). The stream is `text/event-stream` with the four frames as one `oneOf`, which `openapi-typescript` reads as a union. `DoneFrame.summary` is the `oneOf` of the 16 summaries. Five mutants, five caught |
  | **E · stage 5** - `openapi-typescript`; `api.d.ts` generated and committed with the spec's sha256 in its header; `make frontend` regenerates and diffs; a Python test checks the header hash; `main.tsx`'s cast deleted | a read is a type reference | ⛔ **BLOCKED 2026-09-03 (P198) ON A RULING.** `npm install --save-dev openapi-typescript@^7.13.0` is refused: *`peer typescript@"^5.x" from openapi-typescript@7.13.0`* against `typescript@7.0.2`, and the range is exact, not stale - the TypeScript 7 package ships **five files and no compiler API** (`ls node_modules/typescript/lib` → `getExePath.js tsc.js version.cjs ...`), and the generator builds a TypeScript AST through that API; run against it in a scratch directory it crashed, under 5.9.3 it produced 3,793 lines in 143 ms. **Proved with that output, uncommitted**: the cast replaced by `components["schemas"]["OrganizeDoneSummary"] \| ["CompletionBase"]`, `tsc --noEmit` clean (the island reads no field); a probe reading `organised` → *TS2551 Did you mean 'organized'?*, reading `cancelled` → *TS2339* - **`cancelled` is a browser-side addition** (`app.js` spreads it in on cancel; the wire carries `DoneFrame.status`), the `ok` finding's class. Under the old cast every read typed as `{}`. **Two shapes for the maintainer**: (b) a pinned tooling sub-package holding `typescript@5` and `openapi-typescript`, generating into `src/generated/`; (c) a Python emitter of `api.d.ts` from `openapi.json`, which makes the drift check catch hand edits too and adds no Node dependency, accepted against today's openapi-typescript output |
