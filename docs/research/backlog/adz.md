...
- **(adz) A COMPATIBILITY PATH STATES ITS REMOVAL CONDITION WHEN IT IS WRITTEN.** Recorded
  ...
  **Two things it does not license**, because both were checked before the rule was written:
  ...
  | 2 | **`CatalogChoiceReason = Literal["override", "legacy", "default"]`** (`app_paths.py:189`) | ✅ **Already dead and I left it.** `grep 'reason="legacy"'` finds **no producer** since `(adw)`. A member no branch can return. Pure removal, zero risk. |
