...
>
> What is **installed** today: React 19, TypeScript, Vite, **and since 2026-08-15 Tailwind v4.3.3
> and shadcn/ui** (eight components, aliasing `tokens.css` via `@theme inline`; preflight
> deliberately not imported). What remains a **target and is NOT installed**: Tauri (there is still
> no `Cargo.toml`). Lucide supplied the artwork for eleven inlined SVG icons and is **still not a
> dependency** - `lucide-react` imports generated by shadcn were replaced rather than installed.
>
> ⚠ The sentence above read *"a target and is NOT installed: Tailwind, shadcn/ui, Tauri"* until
> 2026-08-15. It was true when written and the install falsified it; corrected rather than left,
> because a superseded header exists to keep a record navigable and a wrong one does the opposite.
