...
import pytest
from truestill_cli.cli import _takeout_root_or_none
...
def test_a_directory_is_passed_straight_through(tmp_path: Path) -> None:
    ...
    assert _takeout_root_or_none(extracted, tmp_path / "dest") == extracted
...
def test_pointing_at_one_part_gathers_its_siblings(tmp_path: Path) -> None:
    ...
    root = _takeout_root_or_none(first, tmp_path / "dest")
...
def test_a_refusal_writes_nothing_and_reports_why(
    ...
    result = _takeout_root_or_none(tmp_path / "takeout-20260801T000000Z-001.zip", destination)
...
def test_a_path_that_is_neither_file_nor_directory_is_named(tmp_path: Path) -> None:
    assert _takeout_root_or_none(tmp_path / "nope.zip", tmp_path / "dest") is None
