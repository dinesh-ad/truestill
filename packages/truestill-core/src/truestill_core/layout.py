...
def parse_timeline_template(template: str) -> LayoutTemplate:
    ...
    **stored** values stay leniently parsed, because a library organized before the year-first
    default has a category-first template written down (`layout_settings.pin_existing_layout`)
    and must keep resolving. Rejecting at load would break exactly the libraries the pin exists
    to protect.
    """
